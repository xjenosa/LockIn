-- Buzzer — playtest fixes migration. Safe to paste into the Supabase SQL editor
-- more than once: every statement is guarded. Live rooms keep working; new
-- columns backfill to sane defaults.

-- =========================== schema ========================================

alter table rooms add column if not exists buzzer_arms_at timestamptz;              -- server-authoritative moment the buzzer goes live
alter table rooms add column if not exists pick_order uuid[] not null default '{}'; -- rotation ring of team ids
...
-- the correct answer, so this gets the final_submissions treatment: RLS on,
-- zero select policies, host reads it through host_get_score_log only.
...
-- Deliberately NOT added to supabase_realtime: the host polls it via RPC, and
-- broadcasting it would push answer text at every connected phone.

-- =========================== superseded overloads ==========================
-- Postgres keys functions on argument types, so adding a defaulted parameter
-- would leave the old signature behind and make PostgREST calls ambiguous.
...
-- Anti-spam: the buzzer arms a few seconds after it opens, and early taps are
-- silently dropped. Enforced here because a client-side check is just a dare.
